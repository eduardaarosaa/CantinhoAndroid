...
public class RecipeGridAdapter extends RecyclerView.Adapter<RecipeGridAdapter.ViewHolder> implements Filterable {
    ...
    public RecipeGridAdapter(Context ctx, List<Publicacoes> items) {
        this.ctx = ctx;
        this.items = items;
...
//        final Recipe p = filtered_items.get(position);
        ...
        final Publicacoes publicacoes = items.get(position);

        holder.name.setText(publicacoes.getTitulo());
//        holder.duration.setText(p.getDuration());
        Picasso.with(ctx).load(publicacoes.getImagem()).into(holder.image);
        ...
        holder.lyt_parent.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if (!clicked && mOnItemClickListener != null) {
                    clicked = true;
                    mOnItemClickListener.onItemClick(view, publicacoes, position);
                }
            }
        });
        clicked = false;
    ...
    public interface OnItemClickListener {
        void onItemClick(View view, Publicacoes obj, int position);
    }
    ...
    private List<Publicacoes> items;
